...
function newsFeed(): void {
  ...
  if (newsFeeds.length === 0) {
    newsFeeds = store.feeds = makeFeed(getData<NewsFeed[]>(NEWS_URL));
  }
  ...
  for (let i = (store.currentPage - 1) * 10; i < store.currentPage * 10; i++) {
    ...
    } mt-6 rounded-lg shadow-md transtision-colors duration-500 hover:bg-green-100">
      <div class="flex">
        <div class="flex-auto">
...
function newsDetail(): void {
  ...
  const newsContent = getData<NeswDetail>(CONTENT_URL.replace("@id", id));
